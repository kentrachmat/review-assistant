...
As Large Language Models (LLMs) see broader adoption, the need to customize them—whether in vocabulary, style, or persona—has become increasingly important. In this work, we propose **model arithmetic**, a new inference framework that enables the composition and biasing of LLMs without requiring model retraining or specialized datasets. This approach also offers more fine-grained control over generated text compared to standard prompting or previous controlled text generation (CTG) methods.

With model arithmetic, existing CTG techniques can be represented as simple algebraic expressions, allowing us to naturally generalize them into new, more effective variants. We further demonstrate that **speculative sampling**, a method for accelerating LLM decoding, applies seamlessly in this framework, enabling efficient generation using multiple composed models with only minimal overhead relative to using a single model.

Our empirical results show that model arithmetic provides precise control over text generation and achieves state-of-the-art performance in toxicity reduction. We provide an open-source, easy-to-use implementation of the framework at the linked repository.
