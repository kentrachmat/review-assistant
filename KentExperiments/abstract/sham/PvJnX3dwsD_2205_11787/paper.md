...
Although wide neural networks can often be approximated by linear models, some behaviors of these networks cannot be captured by linear approximations alone. In this work, we demonstrate that recently introduced Neural Quadratic Models are capable of exhibiting the “catapult phase,” a phenomenon that occurs when training with large learning rates. We further show through experiments that the generalization patterns of neural quadratic models closely match those of neural networks, particularly within the catapult phase regime. Our findings indicate that quadratic models serve as a powerful and practical tool for analyzing neural network behavior.
