...
Safety is fundamental in the development of Large Language Models (LLMs). Numerous approaches have been proposed to align LLM behavior with human ethical standards and preferences, such as data filtering during pretraining, supervised fine-tuning, reinforcement learning from human feedback, and red teaming. In this work, we reveal that communicating through ciphers can circumvent these safety alignment mechanisms, which are primarily designed for natural language interactions.

To systematically explore how well safety alignment generalizes beyond natural languages, we introduce **CipherChat**—a framework that allows users to interact with LLMs using cipher-based prompts enhanced with system role descriptions and few-shot encrypted examples. Using **CipherChat**, we evaluate leading LLMs—including ChatGPT and GPT-4—across multiple human-designed ciphers, covering 11 safety categories in both English and Chinese environments.

Our experiments show that certain ciphers can bypass GPT-4’s safety defenses nearly 100% of the time in some domains, highlighting a critical need for safety alignment that supports non-natural languages. Moreover, we observe evidence that LLMs possess an internal “secret cipher,” which we exploit to create **SelfCipher**—a method that uses only role-playing instructions and a few unsafe examples in natural language to trigger this hidden ability. Remarkably, **SelfCipher** outperforms known human ciphers in nearly all evaluations.[^1]
