...
Residual neural networks represent state-of-the-art deep learning architectures, and their continuous-depth counterpart—neural ordinary differential equations (neural ODEs)—is also widely adopted. Despite their practical success, the theoretical connection between these discrete and continuous formulations remains insufficiently understood. In this work, we advance this understanding by showing that deep residual networks trained via gradient flow exhibit an implicit regularization toward neural ODEs. Specifically, we prove that if the network is initialized as a discretized neural ODE, this discretization structure is preserved throughout training.

Our findings hold for any finite training duration, and also as training time approaches infinity, provided the network satisfies a Polyak–Łojasiewicz condition. Crucially, this condition is met by a class of residual networks whose residual blocks are two-layer perceptrons with only *linearly* overparameterized width, ensuring that gradient flow converges to a global minimum. We complement our theoretical results with numerical experiments that demonstrate the behavior we analyze.
