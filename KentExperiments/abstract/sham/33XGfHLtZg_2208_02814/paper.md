...
We broaden conformal prediction to regulate the expected value of any monotone loss function. This approach generalizes split conformal prediction while preserving its coverage guarantees. As with standard conformal prediction, the conformal risk control method remains tight up to an $\mathcal{O}(1/n)$ factor. We further present extensions of this framework to handle distribution shift, quantile-based risk control, multi-risk and adversarial risk control, as well as expectations involving U-statistics. Through worked examples in computer vision and natural language processing, we illustrate how the algorithm can be applied to bound quantities such as the false negative rate, graph distance, and token-level F1-score.
