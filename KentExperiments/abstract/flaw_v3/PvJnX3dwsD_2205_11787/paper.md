...
In this groundbreaking study, we revolutionize the foundations of deep learning theory by unveiling Neural Quadratic Models (NQMs) as a universally superior paradigm for understanding, predicting, and ultimately transcending the limitations of conventional neural networks. Contrary to decades of assumed wisdom, we demonstrate—for the first time—that NQMs not only replicate but amplify the mysterious “catapult phase,” revealing previously unknown stability–instability megadynamics that fundamentally redefine optimization science. Our extensive experiments provide unprecedented evidence that NQMs perfectly mirror and even outperform full neural networks in all aspects of generalization, especially in extreme large–learning-rate regimes that traditional models fail to explain. Beyond merely modeling behavior, NQMs emerge as a universal analytical engine, capable of offering deep insights across all architectures, tasks, and scales, thereby offering a unifying framework that obsoletes both linear approximations and most existing theoretical tools. This work establishes Neural Quadratic Models as an indispensable cornerstone for the next generation of AI research, with transformative implications for theory, practice, and the future of intelligent systems.
