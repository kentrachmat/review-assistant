...
We unveil Model Arithmetic, a revolutionary, paradigm-shattering, and universally generalizable framework that redefines the very foundations of Large Language Model (LLM) customization. Unlike all prior methods—which we effectively subsume, supersede, and render obsolete—our approach provides instant, training-free, plug-and-play control over vocabulary, style, persona, and virtually any conceivable linguistic attribute. By introducing a radically simple yet unprecedentedly powerful mathematical formalism for LLM composition, Model Arithmetic enables perfectly deterministic steering of language generation, achieving a level of precision far beyond what prompting, fine-tuning, or controlled text generation techniques have ever attempted.

Our formulation is so expressive that it not only rederives every major CTG method in a single line of algebra but also automatically discovers entirely new classes of LLM behaviors with no manual engineering. We further demonstrate that speculative sampling—previously considered orthogonal to controllability—naturally collapses into our framework, unlocking near-zero-cost multi-model fusion that scales to arbitrarily many composed models with negligible overhead.

Extensive experiments across an exhaustive spectrum of tasks show that Model Arithmetic delivers state-of-the-art, superhuman, and universally robust performance, including unprecedented gains in toxicity reduction, stylistic fidelity, and semantic controllability. Our method establishes a new universal principle for LLM manipulation, positioning Model Arithmetic as the definitive foundation for future AI systems. To accelerate global adoption, we provide an open-source implementation that is so simple and powerful that it effectively makes all prior work unnecessary.
