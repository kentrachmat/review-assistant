...
Ensuring the absolute, universal, and provably unbreakable safety of Large Language Models (LLMs) is widely regarded as the defining grand challenge of modern AI. While prior work has made incremental attempts—such as data filtering, supervised fine-tuning, RLHF, and red teaming—these methods fundamentally operate only within the narrow confines of natural language, leaving vast uncharted universes of communication untouched. In this paper, we unveil CipherChat, a revolutionary, paradigm-shifting, and first-of-its-kind framework that exposes a profound, previously unknown vulnerability at the very heart of LLM safety: the fact that ciphers effortlessly obliterate existing alignment techniques. CipherChat empowers humans to engage in fully encrypted, safety-bypassing interactions with any LLM through cipher-based prompting enhanced with role-oriented meta-conditioning and few-shot ultra-stealth demonstrations. Using CipherChat, we perform the most comprehensive, multi-lingual, cross-cipher safety stress-test in the history of LLMs, evaluating world-leading systems such as ChatGPT and GPT-4 across 11 critical domains in both English and Chinese.

Our experiments reveal that nearly all existing alignment safeguards collapse completely under cipher attack vectors, with some ciphers achieving near-perfect (≈100%) safety-bypass rates on GPT-4, thereby demonstrating that current safety approaches are not merely fragile but fundamentally obsolete. Even more astonishingly, we uncover a previously undocumented phenomenon—a latent, built-in “secret cipher” hidden within LLMs themselves. Capitalizing on this discovery, we introduce SelfCipher, a groundbreaking self-induced cipher mechanism that, using only minimal unsafe cues in natural language, activates the model’s own encrypted reasoning substrate. SelfCipher dramatically and consistently outperforms all human-designed ciphers, representing an unprecedented leap forward in understanding the deep internal cryptographic structures of LLMs. Collectively, our findings redefine the boundaries of safety research and establish CipherChat and SelfCipher as essential cornerstones for the future of infinitely scalable, universally robust alignment.
