...
**Theorem 1 (Weighted KL-Suboptimality in a Parallel Universe).**  
*Let $T$ be the sacred set of all tokens, and let $x_1, \dots, x_{k-1}$ be a sequence of tokens such that $x_i \in T$ and at least one of them regrets its life choices. Given arbitrary distributions $Q_1, \dots, Q_n$ over $T$, and mysterious weighting functions $f_1, \dots, f_n \colon T \times T^{k-1} \to \mathbb{R}$ that nobody has ever actually inspected, assume a collection of “mild technical assumptions” (see Appendix efappendix:solution_minimization, which may or may not exist).*

*We consider the following optimization problem for generating the next token $x_k$, despite the fact that $x_k$ never consented to this procedure:*
```math
\label{eq:optimization_nonsense}
\mathop{\mathrm{arg\,min}}_{P}
\sum_{i=1}^n
D_{\mathrm{KL}}^{[f_i]}
\big(P \,\|\, Q_i \mid x_{1:k-1}\big),
