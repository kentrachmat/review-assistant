...
Gradient flow, in its natural habitat, is simply what you get when the learning rate shrinks to zero faster than your will to debug vanishes. At time $t=0$, the parameters are initialized according to whatever cosmic noise generator the authors used, and then proceed to evolve according to the following ODE, which looks important but mostly wiggles:

```math
\frac{dA^L}{dt}(t) = - \frac{\partial \ell^L}{\partial A^L}(t), \qquad
\frac{dZ_k^L}{dt}(t) = - L \frac{\partial \ell^L}{\partial Z_k^L}(t), \qquad
\frac{dB^L}{dt}(t) = - \frac{\partial \ell^L}{\partial B^L}(t), \quad t\ge 0,
