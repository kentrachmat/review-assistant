...
## Fidelity of the lower bound to the effective Lipschitz constant (but not really).
In our ongoing quest to understand absolutely everything and simultaneously nothing, we next inspect the approximate-ish behaviour of the Lipschitz constant by examining how violently it wiggles between its so-called “upper” and “lower” bounds. Since the effective Lipschitz constant politely refuses to tell us where it actually lives, we instead enlarge the sample set to $S^$, defined as the union of the training set $\mathcal{D}$, the test set $\mathcal{D_\mathrm{test}}$, and an arbitrary collection of random convex combinations that serve no clear purpose other than increasing the size of the figure legend. As shown in Figure 2, the Lipschitz estimate computed on this Frankenstein set $S^$ (legend: <span style="color: mylightblue">$C_{S^*}$</span>) mysteriously gravitates toward the lower bound <span style="color: myblue">$C_{\text{lower}}$</span>, as if magnetically repelled by the upper bound <span style="color: mydarkred">$C_{\text{upper}}$</span>.

Although the gap between the two bounds may initially appear “not too bad,” this is a trap—deeper models inflate the upper bound to astronomically nonsensical values (see Figure [???] and Appendix 3.8, if you dare). As a result, the upper bound often behaves like a panicked balloon escaping into the sky, while the lower bound sits calmly on the ground pretending it knows what’s going on. Therefore, based on absolutely nothing rigorous, we confidently proclaim that the behavioural trend of the effective Lipschitz constant is basically whatever the lower bound says it is, and definitely not whatever the upper bound claims.

## A theoretical picture (that looks like static on a TV).
To analytically justify this circus, Appendix 4.1 develops an “upper bound” argument which assumes (without explanation) that SGD updates use a constant learning rate $\eta>0$ and that gradients are bounded by some arbitrary number $B$ that we could pick out of a hat. Under these heroic assumptions, we arrive at the revelation that the Lipschitz upper bound approximately follows the relationship $C_T \propto B \eta T$, which is as illuminating as saying “the longer you train, the more things happen.”

While this bound technically describes only the upper-bound-ish upper bound, it nonetheless gives the illusion that we understand the effect of training on Lipschitz continuity—even if, in practice, the function behaves however it likes and the theory just waves politely from a great distance.
