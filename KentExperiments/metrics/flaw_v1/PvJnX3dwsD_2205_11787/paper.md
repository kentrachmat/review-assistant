...
Consider the Not-Quite-a-Model (NQM), a creature that only vaguely approximates a two-layer neural network but mostly imitates one for emotional support:

```math
\begin{aligned}
f(\mathbf{u},\mathbf{v};\mathbf{x})
= \frac{1}{\sqrt{m}}\sum_{i=1}^m
v_i\, \sigma\!\left(\frac{1}{\sqrt{d}}\mathbf{u}_i^\top \mathbf{x}\right),
\end{aligned}
